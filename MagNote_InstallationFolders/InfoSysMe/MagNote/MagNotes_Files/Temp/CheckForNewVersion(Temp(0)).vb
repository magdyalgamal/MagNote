...
Module CheckForNewVersion
    ...
    ''' Update File DONE 3 = Users Prepaed To Update
    ...
    Public Function Check_For_New_Version_At_Host() As Boolean
        Try
            ...
            LocalFileFullPath = Application.StartupPath & "\UpdateStickyNoteFileInformation.txt"
            SourceFile = "InfoSysMeClients/UpdateStickyNoteFileInformation.txt"
            If Not UpDownloadFile(SourceFile, LocalFileFullPath) Then
                Exit Function
            End If
            If File.Exists(LocalFileFullPath) Then
                Dim UpdateFileInformation() = Split(Replace(Replace(My.Computer.FileSystem.ReadAllText(LocalFileFullPath), vbCrLf, ""), vbLf, ""), ",")
                ...
                ElseIf Val(UpdateFileDONE) = 3 Then 'Update Database Prepared For Users
                    ...
                    End If
                    Dim FileContentsToUpdate = My.Computer.FileSystem.ReadAllText(LocalFileFullPath)
    ...
    Public Function UpDownloadFile(ByVal SourceFile As String, Optional ByVal TargetFile As String = Nothing, Optional ByVal Upload As Boolean = False)
        Try
            ...
            Using client As New WebClient()
                ...
                If Upload Then
                    client.UploadFile("ftp://2728786@p25-preview.awardspace.net/infosysme.com/" & TargetFile, SourceFile)
                Else
                    client.DownloadFile("ftp://2728786@p25-preview.awardspace.net/infosysme.com/" & SourceFile, TargetFile)
                End If
            End Using
            Return True
        Catch ex As Exception
    ...
    Public Function CreatSchedualTask(Optional ByVal DeleteTaskName As String = Nothing, Optional ByVal RestartApplication As Boolean = False) As Boolean
        Try
            ...
            If RestartApplication Then
                CommandLine = Application.StartupPath & "\Sticky_Note.exe
EXIT" & vbNewLine
            Else
                CommandLine = "move /Y " & Update_New_Version_Form.Update_Download_File_Path_TxtBx.Text & "\Sticky_Note-Copy.exe, " & Update_New_Version_Form.Update_Download_File_Path_TxtBx.Text & "\Sticky_Note.exe" & vbNewLine
                CommandLine &= Update_New_Version_Form.Update_Download_File_Path_TxtBx.Text & "\Sticky_Note.exe
EXIT" & vbNewLine
            End If
...
Set WinScriptHost = Nothing"
            ...
            Using ts As New TaskService()
                ...
                td.RegistrationInfo.Description = "Run StickyNote After Downloading New Update"
                ...
                ts.RootFolder.RegisterTaskDefinition("ReRunStickyNote", td)
                Return True
            End Using
        Catch ex As Exception
